...
class TestWopMars(TestCase):
    def setUp(self):
        ...
        self.__def_file = "/home/gonzalez/Software/repositories/wopmars/wopmars/test_bak/resource/wopfile/wopfile1.yml"
        # self.__right_def_file2 = self.s_root_path + "resource/example_def_file4.yml"
    ...
    def test_run(self):
        cmd_line = ["python", "-l", "-D", self.__db_url, "-w", self.__def_file, "-v", "-p", "-d", PathFinder.find_src(os.path.realpath(__file__)), '-F']
    ...
    #                 "-vv", "-p", "-D", self.__db_url, "-d", PathFinder.find_src(os.path.realpath(__file__))]
    #     with self.assertRaises(SystemExit) as se:
    #         WopMars().run(cmd_line)
    #     self.assertEqual(se.exception.code, 0)
    #
