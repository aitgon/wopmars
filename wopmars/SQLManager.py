...
class SQLManager(SingletonMixin):
    ...
    def create(self, tablename):
        ...
        try:
            ...
            Base.metadata.one_x_to_many_tables[tablename.split(".")[-1]].create(self.engine, checkfirst=True)
        finally:
            # Always release the lock
            self.__lock.release()
    ...
    def drop(self, tablename):
        ...
        try:
            ...
            Base.metadata.one_x_to_many_tables[tablename.split(".")[-1]].drop(self.get_engine(), checkfirst=True)
        finally:
            # Always release the lock
            self.__lock.release()
    ...
    def drop_table_list(self, list_str_table):
        ...
        # todo tabling
        list_obj_table = reversed(sort_tables([Base.metadata.one_x_to_many_tables[tablename.split(".")[-1]] for tablename in list_str_table]))
        try:
            self.__lock.acquire_write()
    ...
    def drop_table_content_list(self, list_str_table):
        ...
        list_obj_table = reversed(
            sort_tables([Base.metadata.one_x_to_many_tables[tablename.split(".")[-1]] for tablename in list_str_table]))
        for t in list_obj_table:
