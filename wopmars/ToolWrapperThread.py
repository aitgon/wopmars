...
class ToolWrapperThread(threading.Thread, Observable):
    ...
    def get_command_line(self):
        ...
        """
        list_str_inputs_files = [f.name + "': '" + f.path for f in self.__tool_wrapper.files if f.one_typeio_to_many_x.is_input == 1]
        list_str_inputs_tables = [t.table_name + "': '" + t.model_py_path for t in self.__tool_wrapper.tables if t.one_typeio_to_many_x.is_input == 1]
        str_input_dict = ""
        ...
        list_str_outputs_files = [f.name + "': '" + f.path for f in self.__tool_wrapper.files if f.one_typeio_to_many_x.is_input == 0]
        list_str_outputs_tables = [t.table_name + "': '" + t.model_py_path for t in self.__tool_wrapper.tables if t.one_typeio_to_many_x.is_input == 0]
        str_output_dict = ""
