...
class ToolWrapper(Base):
    ...
    def is_input_respected(self):
        ...
        """
        set_input_file_names = set([f_input.name for f_input in self.relation_typeio_to_fileioinfo if f_input.relation_typeio_to_file_or_tableioinfo.is_input == 1])
        ...
        set_input_table = set([t_input for t_input in self.relation_typeio_to_tableioinfo if t_input.relation_typeio_to_file_or_tableioinfo.is_input == 1])
    ...
    def is_output_respected(self):
        ...
        """
        if set([f_output.name for f_output in self.relation_typeio_to_fileioinfo if f_output.relation_typeio_to_file_or_tableioinfo.is_input == 0]) != set(self.specify_output_file()):
            raise WopMarsException("The content of the definition file is not valid.",
                                   ...
                                   "\n" + "They are:" +
                                   "\n\t'{0}'".format("'\n\t'".join([f.name for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 0]))
                                   )

        set_output_table = set([t_output for t_output in self.relation_typeio_to_tableioinfo if t_output.relation_typeio_to_file_or_tableioinfo.is_input == 0])
    ...
    def follows(self, other):
        ...
        * FileInputOutputInformation
        ...
        """
        for rule_f_path in [f.path for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            for rule_f2_path in [f.path for f in other.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
                if rule_f_path == rule_f2_path:
                    return True

        for rule_t_name in [t.model_py_path for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            for rule_t2_name in [t.model_py_path for t in other.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
    ...
    def get_input_files_not_ready(self):
        ...
        input_files = [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 1]
    ...
    def are_inputs_ready(self):
        ...
        input_tables = [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 1]
    ...
    def set_args_time_and_size(self, type, dry=False):
        ...
        for f in [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == type]:
            try:
        ...
        for t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == type]:
            t.mtime_human = t.relation_tablemodiftime_to_tableioinfo.mtime_human
            t.mtime_epoch_millis = t.relation_tablemodiftime_to_tableioinfo.mtime_epoch_millis
            # t.used_at = t.relation_tablemodiftime_to_tableioinfo.mtime_epoch_millis
            session.add(t)
        session.commit()
    ...
    def same_input_than(self, other):
        ...
        :type other: :class:`~.wopmars.framework.database.relation_typeio_to_tableioinfo.ToolWrapper.ToolWrapper`

        :return: bool
        """
        for t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            is_same = False
            for t2 in [t2 for t2 in other.relation_typeio_to_tableioinfo if t2.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
        ...
        for f in [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            is_same = False
            for f2 in [f2 for f2 in other.relation_typeio_to_fileioinfo if f2.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
    ...
    def is_output_more_recent_than_input(self):
        ...
        """
        most_recent_input = max([get_mtime(f.path)[0] for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 1]
                                + [t.relation_tablemodiftime_to_tableioinfo.mtime_epoch_millis for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 1])
        oldest_output = min([get_mtime(f.path)[0] for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 0]
                            + [t.relation_tablemodiftime_to_tableioinfo.mtime_epoch_millis for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 0])
    ...
    def same_output_than(self, other):
        ...
        :return: bool
        """
        for t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
            is_same = False
            for t2 in [t2 for t2 in other.relation_typeio_to_tableioinfo if t2.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
    ...
    def does_output_exist(self):
        ...
        """
        for of in [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
            if not os.path.exists(of.path):
                return False

        for ot in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
    ...
    def same_files(self, other, is_name):
        ...
        """
        for f in [rf for rf in self.relation_typeio_to_fileioinfo if rf.relation_typeio_to_file_or_tableioinfo.is_input == is_name]:
            is_in = bool([rf for rf in other.relation_typeio_to_fileioinfo if (os.path.abspath(f.path) == os.path.abspath(rf.path) and
                                                       f.name == rf.name and
                                                       rf.relation_typeio_to_file_or_tableioinfo.is_input == is_name)])
            if not is_in:
                return False
        return True
    ...
    def same_tables(self, other, is_input):
        ...
        """
        for t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == is_input]:
            is_in = bool([t for t in other.relation_typeio_to_tableioinfo if (t.model_py_path == t.model_py_path and
                                                      t.relation_typeio_to_file_or_tableioinfo.is_input == is_input and
    ...
    def __repr__(self):
        ...
        s += "\\n"
        for input_f in [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            s += "\\n\t\t" + input_f.name + ": " + str(input_f.path)
        for input_t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 1]:
            s += "\\n\t\tinput_table: " + input_t.name
        s += "\\n"
        for output_f in [f for f in self.relation_typeio_to_fileioinfo if f.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
            s += "\\n\t\t" + output_f.name + ": " + str(output_f.path)
        for output_t in [t for t in self.relation_typeio_to_tableioinfo if t.relation_typeio_to_file_or_tableioinfo.is_input == 0]:
    ...
    def dot_label(self):
        ...
        inputs_list_str = [str(i).replace(":", "") for i in self.relation_typeio_to_fileioinfo + self.relation_typeio_to_tableioinfo if i.relation_typeio_to_file_or_tableioinfo.is_input == 1]
        outputs_list_str = [str(o).replace(":", "") for o in self.relation_typeio_to_fileioinfo + self.relation_typeio_to_tableioinfo if o.relation_typeio_to_file_or_tableioinfo.is_input == 0]
    ...
    def __str__(self):
        inputs_list_str = [str(i) for i in self.relation_typeio_to_fileioinfo + self.relation_typeio_to_tableioinfo if i.relation_typeio_to_file_or_tableioinfo.is_input == 1]
        outputs_list_str = [str(o) for o in self.relation_typeio_to_fileioinfo + self.relation_typeio_to_tableioinfo if o.relation_typeio_to_file_or_tableioinfo.is_input == 0]
    ...
    def input_file(self, key):
        ...
        """
        try:
            return [f.path for f in self.relation_typeio_to_fileioinfo if f.name == key and f.relation_typeio_to_file_or_tableioinfo.is_input == 1][0]
        except IndexError:
    ...
    def input_table(self, key):
        ...
        """
        try:
            return [t for t in self.relation_typeio_to_tableioinfo if t.table_name == key and t.relation_typeio_to_file_or_tableioinfo.is_input == 1][0].get_table()
        except IndexError:
            raise WopMarsException("Error during the execution of the ToolWrapper " + str(self.tool_python_path) +
                                   " (rule " + self.rule_name + ").",
    ...
    def output_file(self, key):
        ...
        """
        try:
            return [f.path for f in self.relation_typeio_to_fileioinfo if f.name == key and f.relation_typeio_to_file_or_tableioinfo.is_input == 0][0]
        except IndexError:
            raise WopMarsException("Error during the execution of the ToolWrapper " + str(self.tool_python_path) +
                                   " (rule " + self.rule_name + ").",
    ...
    def output_table(self, key):
        ...
        """
        try:
            return [t for t in self.relation_typeio_to_tableioinfo if t.table_name == key and t.relation_typeio_to_file_or_tableioinfo.is_input == 0][0].get_table()
        except IndexError:
            raise WopMarsException("Error during the execution of the ToolWrapper " + str(self.tool_python_path) +
                                   " (rule " + self.rule_name + ").",
